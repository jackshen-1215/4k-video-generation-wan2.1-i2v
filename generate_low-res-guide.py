    ...
    if rank == 0:
        ...
        if "t2i" in args.task:
            ...
            cache_video(
                ...
                normalize=True,
                value_range=(-1, 1))
            if low_res_video is not None:
                base, ext = os.path.splitext(args.save_file)
                low_res_save_file = f"{base}_low_res{ext}"
                logging.info(f"Saving low-resolution guidance video to {low_res_save_file}")
                cache_video(
                    tensor=low_res_video[None],
                    save_file=low_res_save_file,
                    fps=cfg.sample_fps,
                    nrow=1,
                    normalize=True,
                    value_range=(-1, 1))
